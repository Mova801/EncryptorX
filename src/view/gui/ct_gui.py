...
from src.view.view_constants import AppConstants, ButtonConstants, LabelConstants, TextBoxConstants, FontConstants
...
class CTGUI(ct.CTk, AbstractView):
    ...
    def __init__(self, title: str, win_size: tuple[int, int]) -> None:
        ...
        self.iconbitmap(Path.cwd().joinpath(AppConstants.image_path).joinpath(AppConstants.logo))

        # setting window theme and appearance
        ct.set_appearance_mode(AppConstants.appearance_mode)
        ct.set_default_color_theme(AppConstants.color_theme)
    ...
    def build(self, controller) -> None:
        ...
        self.__input_label.grid(
            ...
            padx=AppConstants.inner_padx, pady=AppConstants.inner_pady,
        ...
        image = PIL.Image.open(Path.cwd().joinpath(AppConstants.image_path).joinpath(AppConstants.bug_icon))
        ...
        self.__bug_button = ct.CTkButton(
            ...
            hover_color=ButtonConstants.hover_color,
            font=(FontConstants.font, FontConstants.size_M),
            image=ct.CTkImage(dark_image=image, size=AppConstants.icon_size),
            command=lambda: controller.handle_hyperlink_request(controller_constants.RequestType.BUG_REPORT)
        )
